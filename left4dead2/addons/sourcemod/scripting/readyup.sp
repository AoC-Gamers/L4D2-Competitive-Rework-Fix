...
{
	UnhookEvent("round_start", RoundStart_Event, EventHookMode_Pre);
	UnhookEvent("player_team", PlayerTeam_Event, EventHookMode_Post);
	UnhookEvent("gameinstructor_draw", GameInstructorDraw_Event, EventHookMode_PostNoCopy);

	l4d_ready_survivor_freeze.RemoveChangeHook(CvarChg_SurvFreeze);
	l4d_ready_server_cvar.RemoveChangeHook(CvarChg_ServerCvar);

	ToggleCommandListeners(false);

	for (int client = 1; client <= MaxClients; client++)
	{
		delete g_hChangeTeamTimer[client];
		g_hChangeTeamTimer[client] = null;
	}

	delete nativeFooter;
	delete g_hPreInitiateForward;
	delete g_hInitiateForward;
	delete g_hPreCountdownForward;
	delete g_hCountdownForward;
	delete g_hPreLiveForward;
	delete g_hLiveForward;
	delete g_hCountdownCancelledForward;
	delete g_hPlayerReadyForward;
	delete g_hPlayerUnreadyForward;
