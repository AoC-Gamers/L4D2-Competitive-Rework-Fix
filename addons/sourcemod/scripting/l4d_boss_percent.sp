...
GlobalForward g_hBossPercentsUpdated;
int g_iLastForwardTankPercent = -2;
int g_iLastForwardWitchPercent = -2;

public void OnPluginStart()
{
	LoadTranslations("l4d_boss_percent.phrases");

	g_hBossPercentsUpdated = new GlobalForward("L4D2_OnBossPercentsUpdated", ET_Ignore, Param_Cell, Param_Cell);
...
void BroadcastBossPercentsUpdated()
{
	if (g_hBossPercentsUpdated == null)
		return;

	if (g_iLastForwardTankPercent == g_fTankPercent && g_iLastForwardWitchPercent == g_fWitchPercent)
		return;

	g_iLastForwardTankPercent = g_fTankPercent;
	g_iLastForwardWitchPercent = g_fWitchPercent;

	Call_StartForward(g_hBossPercentsUpdated);
	Call_PushCell(g_fTankPercent);
	Call_PushCell(g_fWitchPercent);
	Call_Finish();
}
