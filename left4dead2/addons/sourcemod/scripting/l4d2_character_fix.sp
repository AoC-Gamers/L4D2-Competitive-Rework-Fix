...
public void OnPluginStart()
{
	...
	g_cvMaxZombies = FindConVar("z_max_player_zombies");
}

Action TeamCmd(int iClient, const char[] sCommand, int iArgc)
{
	if (iClient <= 0 || iClient > MaxClients || !IsClientInGame(iClient) || iArgc < 1) {
	...
	if (GetClientTeam(iClient) == TEAM_SURVIVOR 
		...
		if (GetInfectedCount() >= g_cvMaxZombies.IntValue) {
